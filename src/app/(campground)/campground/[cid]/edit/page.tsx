...
export default function EditCampground({ params }: { params: { cid: string } }) {
  ...
  const [editAmenities, setEditAmenities] = useState<AmenityItem[]>([]);
  ...
  const amenityEdit = (amenity:AmenityItem) =>{
    if(amenity){
      const existing = editAmenities.findIndex(item => 
          (item._id === amenity._id ))
      if (existing>=0) {
          const updatedList = [...editAmenities];
          ...
          <div>
            {
              amenities.map((amenity)=>(
                <AmenityEditItem amenities={amenity} handleSubmit={amenityEdit} handleDelete={amenityDelete}/>
              ))
            }
          </div>
        </div>
